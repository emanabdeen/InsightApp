...
namespace InsightApp.Tests.ControllerTests
{
    public class FriendsTests : IDisposable
    {
        ...
        [Fact]
        public async Task AddFriend_ReturnsGetFriendPageWithFriendAdded_Success()
        {
            var context = CreateContext();
            int expectedFriendId = 1;
            int expectedFriendCount = 1;
            ...
            //Assert
            Assert.True(model.Count == expectedFriendCount);
            Assert.True(model[0].FriendId == expectedFriendId);
            Assert.True(model[0].UserName == expectedFriendName);
        }

        [Fact]
        public async Task AddFriendThenRemoveFriend_ReturnsGetFriendsModelWithNoFriends_Success()
        {
            var context = CreateContext();
            int friendId = 1;
            int expectedFriendCount = 0;
            OptionsWrapper<IdentityOptions> optionsWrapper = new OptionsWrapper<IdentityOptions>(new IdentityOptions());
            ProgramConstants.GetIdentityOptions()(optionsWrapper.Value);
            var userValidators = new List<IUserValidator<Account>> { new UserValidator<Account>() };
            var passwordValidator = new List<IPasswordValidator<Account>> { new PasswordValidator<Account>() };
            ...
            //Act
            var addFriendResult = await controller.AddFriend(friendId) as RedirectToActionResult;
            Assert.True(addFriendResult.ActionName == "GetFriends");
            var addedFriendGetFriendListResult = await controller.GetFriends() as ViewResult;
            var addedFriendModel = addedFriendGetFriendListResult.Model as List<FriendViewModel>;
            Assert.True(addedFriendModel.Count == 1);
            var removeFriendResult = await controller.RemoveFriend(friendId) as RedirectToActionResult;
            Assert.True(removeFriendResult.ActionName == "GetFriends");
            var getFriendsListResult = await controller.GetFriends() as ViewResult;
            var model = getFriendsListResult.Model as List<FriendViewModel>;
            Assert.NotNull(model);
